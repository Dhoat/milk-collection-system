<x-admin-layout>
    <x-slot name="header">
        <x-admin.page-header 
            title="{{ __('Edit Farmer') }}: {{ $farmer->name }}" 
            description="{{ __('Update producer profile, mobile contact, and banking information') }}">
            <x-slot name="actions">
    ...
    <div class="w-full space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 lg:p-8 space-y-8">
            <form method="POST" action="{{ route('farmers.update', $farmer) }}" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="farmer_code" name="farmer_code" type="text" class="w-full py-2.5 px-3.5 text-xs font-mono font-bold border-slate-200 rounded-xl shadow-xs" :value="old('farmer_code', $farmer->farmer_code)" required autocomplete="off" />
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="w-full py-2.5 px-3.5 text-xs font-medium border-slate-200 rounded-xl shadow-xs" :value="old('name', $farmer->name)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="father_name" name="father_name" type="text" class="w-full py-2.5 px-3.5 text-xs font-medium border-slate-200 rounded-xl shadow-xs" :value="old('father_name', $farmer->father_name)" />
                        ...
                        <div>
                            ...
                            <x-text-input id="joining_date" name="joining_date" type="date" class="w-full py-2.5 px-3.5 text-xs font-mono border-slate-200 rounded-xl shadow-xs" :value="old('joining_date', $farmer->joining_date ? $farmer->joining_date->format('Y-m-d') : '')" />
                        ...
                        <div>
                            ...
                            <x-text-input id="mobile" name="mobile" type="text" class="w-full py-2.5 px-3.5 text-xs font-mono border-slate-200 rounded-xl shadow-xs" :value="old('mobile', $farmer->mobile)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="alternate_mobile" name="alternate_mobile" type="text" class="w-full py-2.5 px-3.5 text-xs font-mono border-slate-200 rounded-xl shadow-xs" :value="old('alternate_mobile', $farmer->alternate_mobile)" />
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="2" class="w-full py-2.5 px-3.5 text-xs font-medium border-slate-200 focus:border-[#005BAC] focus:ring-2 focus:ring-[#005BAC]/20 rounded-xl shadow-xs">{{ old('address', $farmer->address) }}</textarea>
                    ...
                    <div x-show="open" x-collapse class="space-y-5 pt-2">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="bank_name" name="bank_name" type="text" class="w-full py-2.5 px-3.5 text-xs font-medium border-slate-200 rounded-xl shadow-xs bg-white" :value="old('bank_name', $farmer->bank_name)" placeholder="e.g. State Bank of India" />
                            ...
                            <div>
                                ...
                                <x-text-input id="account_number" name="account_number" type="text" class="w-full py-2.5 px-3.5 text-xs font-mono border-slate-200 rounded-xl shadow-xs bg-white" :value="old('account_number', $farmer->account_number)" placeholder="e.g. 1234567890" />
                            ...
                            <div>
                                ...
                                <x-text-input id="ifsc_code" name="ifsc_code" type="text" class="w-full py-2.5 px-3.5 text-xs font-mono uppercase border-slate-200 rounded-xl shadow-xs bg-white" :value="old('ifsc_code', $farmer->ifsc_code)" placeholder="e.g. SBIN0001234" />
                ...
                <div x-data="{ open: true, isActive: {{ old('status', $farmer->status) ? 'true' : 'false' }} }" class="bg-emerald-50/40 border border-emerald-100/80 rounded-2xl p-5 lg:p-6 space-y-5">
                    <div class="flex items-center justify-between cursor-pointer" @click="open = !open">
                        <div class="flex items-center gap-3">
