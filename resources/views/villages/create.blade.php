<x-admin-layout>
    <x-slot name="header">
        <x-admin.page-header 
            title="{{ __('Add New Village') }}" 
            description="{{ __('Register a new collection center sector in the system') }}">
            <x-slot name="actions">
    ...
    <div class="w-full space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-200/70 shadow-xs p-6 lg:p-8 space-y-8">
            <form method="POST" action="{{ route('villages.store') }}" class="space-y-8">
                @csrf
                ...
                <div class="space-y-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400 font-medium">{{ __('Enter the basic details of the village') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input id="code" name="code" type="text" value="{{ old('code') }}" required autofocus placeholder="e.g. VIL001"
                                   class="w-full py-2.5 px-3.5 text-xs font-mono font-medium border border-slate-200 focus:border-[#005BAC] focus:ring-2 focus:ring-[#005BAC]/20 rounded-xl shadow-2xs placeholder:text-slate-300 transition-all" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name') }}" required placeholder="e.g. Greenfield Center"
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="3" required placeholder="Enter complete address or landmark details..."
                                  class="w-full py-2.5 px-3.5 text-xs font-medium border border-slate-200 focus:border-[#005BAC] focus:ring-2 focus:ring-[#005BAC]/20 rounded-xl shadow-2xs placeholder:text-slate-300 transition-all">{{ old('address') }}</textarea>
                    ...
                    <div class="flex items-center justify-between cursor-pointer" @click="open = !open">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-400 font-medium">{{ __('Help us identify the geographical location') }}</p>
                            </div>
                        </div>
                ...
                <div x-data="{ open: true, isActive: true }" class="bg-white border border-slate-200/80 rounded-2xl p-6 space-y-6 shadow-2xs">
                    <div class="flex items-center justify-between cursor-pointer" @click="open = !open">
                        <div class="flex items-center gap-3">
