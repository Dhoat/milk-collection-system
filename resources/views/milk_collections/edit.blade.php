    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Milk Collection') }}: {{ $milkCollection->farmer->name }} ({{ $milkCollection->collection_date->format('d-m-Y') }} - {{ ucfirst($milkCollection->shift) }})
            </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('milk-collections.update', $milkCollection) }}" 
                          x-data="{
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="collection_date" name="collection_date" type="date" class="mt-1 block w-full" :value="old('collection_date', $milkCollection->collection_date->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <select id="shift" name="shift" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    <option value="morning" {{ old('shift', $milkCollection->shift) === 'morning' ? 'selected' : '' }}>{{ __('Morning') }}</option>
                                    <option value="evening" {{ old('shift', $milkCollection->shift) === 'evening' ? 'selected' : '' }}>{{ __('Evening') }}</option>
                                </select>
                        ...
                        <div class="border-t border-gray-100 pt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="fat" name="fat" type="number" step="0.01" class="mt-1 block w-full" :value="old('fat', $milkCollection->fat)" placeholder="e.g. 4.5" autocomplete="off" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="snf" name="snf" type="number" step="0.01" class="mt-1 block w-full" :value="old('snf', $milkCollection->snf)" placeholder="e.g. 8.5" autocomplete="off" />
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="3">{{ old('notes', $milkCollection->notes) }}</textarea>
