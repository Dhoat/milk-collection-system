...
{
    /**
     * Get complete dashboard summary data for a given date.
     */
    public function getDashboardData(?string $date = null): array
    {
        ...
        $todayQuantity = (float) MilkCollection::whereDate('collection_date', $targetDateString)->sum('milk_quantity');
        $todayAmount = (float) MilkCollection::whereDate('collection_date', $targetDateString)->sum('amount');

        $morningQuantity = (float) MilkCollection::whereDate('collection_date', $targetDateString)
            ->where('shift', 'morning')
            ->sum('milk_quantity');
        $morningFarmerCount = MilkCollection::whereDate('collection_date', $targetDateString)
            ->where('shift', 'morning')
            ->distinct('farmer_id')
            ->count('farmer_id');

        $eveningQuantity = (float) MilkCollection::whereDate('collection_date', $targetDateString)
            ->where('shift', 'evening')
            ->sum('milk_quantity');
        $eveningFarmerCount = MilkCollection::whereDate('collection_date', $targetDateString)
            ->where('shift', 'evening')
            ->distinct('farmer_id')
            ->count('farmer_id');

        $todayFarmerCount = MilkCollection::whereDate('collection_date', $targetDateString)
            ->distinct('farmer_id')
            ->count('farmer_id');
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $collectionTrend->push([
                'date' => $dKey,
                'label' => $d->format('D'),
                'full_label' => $d->format('M d'),
                'litres' => (float) ($trendTotals[$dKey] ?? 0),
            ]);
        }
        ...
        $villagePerformance = Village::query()
            ...
            ->leftJoin('milk_collections', function ($join) use ($targetDateString) {
                $join->on('milk_collections.farmer_id', '=', 'farmers.id')
                    ->whereDate('milk_collections.collection_date', $targetDateString);
            })
     ...
     *
     * @return Collection<int, array{type: string, message: string, detail: string, timestamp: \Carbon\Carbon, icon: string}>
     */
